...
def draw_cyberpunk_box(img, x1, y1, x2, y2, label, confidence, color=(0, 255, 170)):
    ...
    # 1. Draw subtle semi-transparent filled box
    overlay = img.copy()
    cv2.rectangle(overlay, (x1, y1), (x2, y2), color, -1)
    cv2.addWeighted(overlay, 0.1, img, 0.9, 0, img)
    
    # 2. Draw border box with very thin line
    cv2.rectangle(img, (x1, y1), (x2, y2), color, 1)
    
    # 3. Draw heavy corner brackets
    ...
    # Draw dark text for contrast on bright neon background
...
def process_video_frames(input_path: str, output_path: str, model_name: str, confidence: float, progress_callback):
    ...
    # 1. Load model (this triggers download if needed)
    ...
    # 3. Setup output video writer using imageio (forces libx264 browser compatibility)
    try:
    ...
    color_fish = (170, 255, 0)       # Cyan-Lime
    color_dolphin = (255, 0, 255)    # Neon Magenta
    color_default = (0, 215, 255)    # Neon Yellow-Gold
    
    # Dynamic frame stride optimization based on video length
    # For high FPS or long videos, sample every Nth frame for inference to boost speed by 5x-10x
    ...
    try:
        while cap.isOpened():
            ...
            # Run YOLO prediction on sampled frames (or frame 1)
            ...
            # Determine HUD text dynamically based on the model
            ...
            # Convert BGR (OpenCV) to RGB (imageio expectations)
