            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div>
                ...
                >
                  ...
                  <option value="albatross">🦅 Albatross & Seabird (YOLO-World)</option>
              ...
              >
                {processing ? `Analyzing (${processStatus.progress}%)...` : (modelName === 'albatross' ? 'Run Albatross Detection' : 'Run Dolphin Detection')}
              </button>
            </div>
          </div>
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '4px', fontSize: '10px', opacity: 0.7 }}>
                      ...
                      <div style={{ display: 'flex', gap: '6px' }}>
                        ...
                        {vid.has_albatross && <span style={{ color: '#f59e0b', fontWeight: 'bold' }}>🦅 Albatross</span>}
                      </div>
                    </div>
                  </div>
                ))
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
          ...
          {!selectedVideo ? (
            ...
            <>
              ...
              {/* Protected Seabird Albatross Alert Banner */}
              {results && (results.has_albatross || results.peak_albatross_count > 0) && (
                <div style={{
                  padding: '16px',
                  borderRadius: 'var(--radius)',
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <div style={{ fontSize: '28px' }}>🦅</div>
                    <div>
                      <div style={{ fontWeight: 'bold', fontSize: '14px', color: '#fde047' }}>
                        PROTECTED SEABIRD ALERT: ALBATROSS DETECTED
                      </div>
                      <div style={{ fontSize: '12px', opacity: 0.9, marginTop: '2px' }}>
                        Identified in {results.total_albatross_frames || 1} frame(s) (Peak Count: {results.peak_albatross_count}). Auto-flagged as <strong style={{ color: '#fde047' }}>Bycatch species (High Severity)</strong> in SQLite database.
                      </div>
                    </div>
                  </div>

                  {results.albatross_events && results.albatross_events.length > 0 && (
                    <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                      <span style={{ fontSize: '10px', opacity: 0.8 }}>Skip to:</span>
                      {results.albatross_events.slice(0, 4).map((ev, i) => (
                        <button
                          key={i}
                          onClick={() => {
                            if (originalVideoRef.current) originalVideoRef.current.currentTime = ev.timestamp;
              ...
              {results && (
                <div style={{
                  display: 'grid',
                  gridTemplateColumns: (results.has_dolphin || results.has_albatross) ? 'repeat(5, 1fr)' : 'repeat(4, 1fr)',
                  gap: 'var(--space-3)'
                }}>
                  {results.has_dolphin && (
                    <div style={{
                  ...
                  {results.has_albatross && (
                    <div style={{
                      ...
                      <div style={{ fontSize: '10px', textTransform: 'uppercase', opacity: 0.8, color: '#fde047' }}>🦅 Albatross Peak</div>
                      <div style={{ fontSize: '22px', fontWeight: '900', color: '#fef08a', marginTop: '2px' }}>{results.peak_albatross_count || 1}</div>
                      <div style={{ fontSize: '9px', color: '#fde047', marginTop: '2px' }}>Protected Seabird</div>
                    </div>
                  )}
